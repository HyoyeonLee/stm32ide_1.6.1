...
//static uint8_t B2D(uint8_t bcd) {return (bcd>>4)*10 + (bcd&0x0F);}
//static uint8_t D2B(uint8_t decimal){return ( ((decimal/10)<<4) | (decimal%10) );}
...
//---------------------------------------------------------------------------------------------------AT24C32 -Mine
bool AT24C32_EEPROM_read(uint16_t start_address, uint8_t* read_buf,int ndata)
{
	if(HAL_I2C_Mem_Read(i2c, 0x57<<1, start_address,  sizeof(uint16_t), read_buf, ndata,  HAL_MAX_DELAY)!=HAL_OK){return false;}
	HAL_Delay(ndata*10);
	return true;
}

bool AT24C32_EEPROM_write(uint16_t start_address, uint8_t* write_buf,int ndata)
{
	if(HAL_I2C_Mem_Write(i2c,0x57<<1, start_address, sizeof(uint16_t), write_buf, ndata,  HAL_MAX_DELAY)!=HAL_OK){return false;}
	HAL_Delay(ndata*10);
	return true;
}
//--------------------------------------------------------------------------------------------------DS3231
...
bool DS3231_GetTime(_RTC* rtc)
{
	...
	start_address = DS3231_REG_TIME;
	read_buf = (uint8_t*)malloc(7*sizeof(uint8_t));

	DS3231_Register_read(start_address,read_buf);
	rtc->Sec 		= B2D(read_buf[0] & 0x7F);  	//0111(sec/10) 	1111(sec%10)
	rtc->Min 		= B2D(read_buf[1] & 0x7F);  	//0111(min/10) 	1111(min%10)
	rtc->Hour 		= B2D(read_buf[2] & 0x3F);  	//0011(hour/10)	1111(hour%10)
	rtc->DaysOfWeek	= B2D(read_buf[3] & 0x07);  	//0000() 		0111(day)
	rtc->Date 		= B2D(read_buf[4] & 0x3F);  	//0011(date/10) 1111(date%10)
	rtc->Month 		= B2D(read_buf[5] & 0x1F);  	//0001(month/10)1111(month%10)
	rtc->Year 		= B2D(read_buf[6]);			  	//1111(year/10) 1111(year%10)
